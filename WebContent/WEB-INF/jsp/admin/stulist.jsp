...
function search(){
	var url = "<c:url value="/admin/student.do?m=stuList"/>";
	var key = $("#key").val();
	if(key!=""){
		url+="&key="+key;
	}
...
<td valign="top">
	...
	<!-- <dd><input type="button" value="添加用户" onclick="location='/classnet/admin/user.do?m=toAdd'"/>
    </dd> -->
	</dl>
	  <table width="99%" border="0" cellpadding="0" cellspacing="1" bgcolor="#dae2e5">
	    <tr>
	      ...
	      <td width="10%" height="30" bgcolor="#e5edfa" align="center">用户名</td>
	      <td width="20%" height="30" bgcolor="#e5edfa" align="center">真实姓名</td>
	      <td width="20%" height="30" bgcolor="#e5edfa" align="center">邮箱</td>
	      <td width="10%" height="30" bgcolor="#e5edfa" align="center">角色类型</td>
		...
		<tr>
	      ...
	      <td height="30" bgcolor="#FFFFFF" align="center"><c:out value="${user.username}"/></td>
	      <td height="30" bgcolor="#FFFFFF" align="center"><%-- <c:out value="${user.password}"/> --%><c:out value="${user.realName}"/></td>
	      <td height="30" bgcolor="#FFFFFF" align="center"><c:out value="${user.email}"/></td>
	      <td height="30" bgcolor="#FFFFFF" align="center">
	      	<c:if test="${user.authorite eq 'ROLE_SUPERVISOR'}">管理员</c:if>
	      	<c:if test="${user.authorite eq 'ROLE_TEACHER'}">教师</c:if>
	      	<c:if test="${user.authorite eq 'ROLE_USER'}">学生</c:if>
	      </td>
	      <td height="30" bgcolor="#FFFFFF" align="center">
	      	<c:choose>
	      ...
	      <td width="10%" height="30" bgcolor="#FFFFFF" align="center">
	      	<c:choose>
	      		...
	      		<a href="<c:url value="/admin/student.do?m=changestatus&id=${user.id}&status=0"/>">停用</a>&nbsp;
	      		恢复
	      		</c:when>
	      		<c:otherwise>
	      		停用&nbsp;
	      		<a href="<c:url value="/admin/student.do?m=changestatus&id=${user.id}&status=1"/>">恢复</a>
	      		</c:otherwise>
	      	</c:choose>
	      	<a href="<c:url value="/admin/student.do?m=toUpdate&id=${user.id}"/>">修改</a>
	      	<a href="<c:url value="/admin/student.do?m=delUser&id=${user.id}"/>">删除</a>
	      </td>
	    </tr>				
	    </c:forEach>
		</c:when>
